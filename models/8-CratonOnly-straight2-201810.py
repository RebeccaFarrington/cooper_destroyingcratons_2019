...
craton_d3_Shape = np.array([ (2.6,-1.6), (2.6, 0.2), (-1.5, 0.2), (-1.5, -1.6) ])
...
#craton_transZ_d1dOL_Shape = np.array([(2.6, 0.5-dTrans),(2.6, 0.5+dTrans),(-1.5, 0.5+dTrans),(-1.5, 0.5-dTrans)])
craton_transZ_d1dOL_Shape = np.array([(2.6, 0.2-dTrans),(2.6, 0.2+dTrans),(-1.5, 0.2+dTrans),(-1.5, 0.2-dTrans)])
craton_transZ_d1d2_Shape  = np.array([(2.6, 0.2-dTrans),(2.6, 0.2+dTrans),(-1.5, 0.2+dTrans),(-1.5, 0.2-dTrans)])
craton_transZ_d2d3_Shape  = np.array([(2.6,-0.1-dTrans),(2.6,-0.1+dTrans),(-1.5,-0.1+dTrans),(-1.5,-0.1-dTrans)])

craton_transZ_d1dOL = fn.shape.Polygon( craton_transZ_d1dOL_Shape )
craton_transZ_d1d2  = fn.shape.Polygon( craton_transZ_d1d2_Shape ) 
craton_transZ_d2d3  = fn.shape.Polygon( craton_transZ_d2d3_Shape ) 

# create transition regions in X direction
# craton_transX_d1dOL_Shape = np.array([ (-1.5-dTrans, 0.2), (-1.5-dTrans, 0.5), (-1.5+dTrans, 0.5), (-1.5+dTrans,  0.2) ])
# craton_transX_d2dOL_Shape = np.array([ (-1.5-dTrans,-0.1), (-1.5-dTrans, 0.2), (-1.5+dTrans, 0.2), (-1.5+dTrans, -0.1) ])
#craton_transX_d3dOL_Shape = np.array([ (-1.5-dTrans,-1.6), (-1.5-dTrans,-0.1), (-1.5+dTrans,-0.1), (-1.5+dTrans, -1.6) ])
#craton_transX_d3dOL_Shape = np.array([ (-1.5-dTrans,-1.6), (-1.5-dTrans, 0.5), (-1.5+dTrans, 0.5), (-1.5+dTrans, -1.6) ])
craton_transX_d3dOL_Shape = np.array([ (-1.5-dTrans,-1.6), (-1.5-dTrans, 0.2), (-1.5+dTrans, 0.2), (-1.5+dTrans, -1.6) ])

# craton_transX_d1dOL  = fn.shape.Polygon( craton_transX_d1dOL_Shape ) 
# craton_transX_d2dOL  = fn.shape.Polygon( craton_transX_d2dOL_Shape ) 
craton_transX_d3dOL  = fn.shape.Polygon( craton_transX_d3dOL_Shape ) 
...
#        temp = min(tempSlab, tempOL) 
    ...
    # if within craton
    if craton_d3.evaluate((coord[0],coord[2])):
...
#        temp = deltaTemp*math.erf(depth/(2.*math.sqrt(kappa*ageD1)))/deltaTemp
    ...
    if craton_transZ_d1dOL.evaluate((coord[0],coord[2])):
        ZtransStart =  0.2-dTrans
        phi  = (coord[2]-ZtransStart)/(2.*dTrans)
        age = (1.-phi) * ageD3 + phi * ageOL  # use D3 age here, no D1 in straight model
...
#        temp = deltaTemp*math.erf(depth/(2.*math.sqrt(kappa*age)))/deltaTemp
    if craton_transX_d3dOL.evaluate((coord[0],coord[2])):
        XtransStart = -1.5-dTrans
        phi  = (coord[0]-XtransStart)/(2.*dTrans)
